...
Value* CminusfBuilder::visit(ASTVarDeclaration &node) {
    ...
    if (node.num == nullptr) { // 标量变量
        if (scope.in_global()) {
            // 全局变量
            auto initializer = node.type == TYPE_INT ? 
                ConstantZero::get(INT32_T, module.get()) : 
                ConstantZero::get(FLOAT_T, module.get());
            alloca = GlobalVariable::create(node.id, module.get(), var_type, false, initializer);
        } else {
            // 局部变量
        ...
        if (scope.in_global()) {
            ...
            alloca = GlobalVariable::create(node.id, module.get(), array_type, false, initializer);
        } else {
            // 局部数组
...
Value* CminusfBuilder::visit(ASTFunDeclaration &node) {
    ...
    for (unsigned int i = 0; i < node.params.size(); ++i) {
        ...
        if (node.params[i]->type == TYPE_INT) {
            param_type = node.params[i]->isarray ? INT32PTR_T : INT32_T;
        } else {
            param_type = node.params[i]->isarray ? FLOATPTR_T : FLOAT_T;
        }
        
        auto param_alloca = builder->create_alloca(param_type);
